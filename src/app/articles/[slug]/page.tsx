...
import { FontSizeControls } from "@/components/articles/FontSizeControls";
import { SocialButtons } from "@/components/articles/SocialButtons";
import { ArticleData, Theme } from "@/interface/interface";
import { ColorSwitcher } from "@/components/articles/ColorSwitcher";
import { getSingleArticle } from "@/utils/get-single-article";
import { motion } from "framer-motion";
...
export default function ArticlePage({ params }: PageProps) {
  ...
  React.useEffect(() => {
    const fetchArticle = async () => {
      try {
        setIsLoading(true);
        const data = await getSingleArticle(params.slug);
        setArticleData(data);
        setError(null);
      } catch (err) {
  ...
  return (
    ...
    >
      ...
      <FontSizeControls
        ...
        currentTheme={currentTheme} // Make sure this is defined
      />
    </div>
  );
}
